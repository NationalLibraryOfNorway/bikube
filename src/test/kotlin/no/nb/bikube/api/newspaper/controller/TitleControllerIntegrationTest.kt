...
class TitleControllerIntegrationTest (
    ...
    fun beforeEach() {
        ...
        every { collectionsRepository.getSingleCollectionsModel(titleId) } returns Mono.just(CollectionsModelMockData.collectionsModelEmptyRecordListMock.copy())
        every { collectionsRepository.getSingleCollectionsModel("2") } returns Mono.just(CollectionsModelMockData.collectionsModelMockTitleC.copy())
        ...
        every { collectionsRepository.getSingleCollectionsModelWithoutChildren("2") } returns Mono.just(
            CollectionsModelMockData.collectionsModelMockTitleC.copy())
        every { collectionsRepository.createNewspaperRecord(any()) } returns Mono.just(CollectionsModelMockData.collectionsModelMockTitleC.copy())
        every { collectionsRepository.searchPublisher(any()) } returns Mono.just(CollectionsModelMockData.collectionsNameModelMockA.copy())
        every { collectionsRepository.searchPublisherPlace(any()) } returns Mono.just(CollectionsModelMockData.collectionsTermModelMockLocationB.copy())
        every { collectionsRepository.searchLanguage(any()) } returns Mono.just(CollectionsModelMockData.collectionsTermModelMockLanguageA.copy())
        every { collectionsRepository.createNameRecord(any(), CollectionsDatabase.PEOPLE) } returns Mono.just(
            CollectionsModelMockData.collectionsNameModelMockA.copy())
        every { collectionsRepository.createTermRecord(any(), CollectionsDatabase.GEO_LOCATIONS) } returns Mono.just(
            CollectionsModelMockData.collectionsTermModelMockLocationB.copy())
        every { collectionsRepository.createTermRecord(any(), CollectionsDatabase.LANGUAGES) } returns Mono.just(
            CollectionsModelMockData.collectionsTermModelMockLanguageA.copy())
        every { uniqueIdService.getUniqueId() } returns "123"
    }

    @Test
    fun `post-newspapers-titles should return 201 created with title`() {
        createTitle(NewspaperMockData.newspaperTitleInputDtoMockA)
            .expectStatus().isCreated
    }

    @Test
    fun `post-newspapers-titles should return correctly mapped title`() {
        createTitle(NewspaperMockData.newspaperTitleInputDtoMockB)
            .expectStatus().isCreated
            .returnResult<Title>()
            .responseBody
            .test()
            .expectSubscription()
            .expectNext(NewspaperMockData.newspaperTitleMockB)
            .verifyComplete()
    }

    @Test
    fun `post-newspapers-titles should return 400 bad request if title is empty`() {
        createTitle(NewspaperMockData.newspaperTitleInputDtoMockA.copy(name = ""))
    ...
    fun `post-newspapers-titles should use publisher if present and in Collections`() {
        ...
        createTitle(NewspaperMockData.newspaperTitleInputDtoMockA.copy(publisher = "publisher"))
            .expectStatus().isCreated
    ...
    fun `post-newspapers-titles should create publisher if present and not in Collections`() {
        every { collectionsRepository.searchPublisher(any()) } returns Mono.just(CollectionsModelMockData.collectionsNameModelWithEmptyRecordListA.copy())
    ...
    fun `post-newspapers-titles should ignore publisher if not present`() {
        createTitle(NewspaperMockData.newspaperTitleInputDtoMockA.copy(publisher = null))
            .expectStatus().isCreated
    ...
    fun `post-newspapers-titles should use publisherPlace if present and in Collections`() {
        ...
        createTitle(NewspaperMockData.newspaperTitleInputDtoMockA.copy(publisherPlace = "Mo"))
            .expectStatus().isCreated
    ...
    fun `post-newspapers-titles should create publisherPlace if present and not in Collections`() {
        every { collectionsRepository.searchPublisherPlace(any()) } returns Mono.just(CollectionsModelMockData.collectionsTermModelWithEmptyRecordListA.copy())
    ...
    fun `post-newspapers-titles should ignore publisherPlace if not present`() {
        createTitle(NewspaperMockData.newspaperTitleInputDtoMockA.copy(publisherPlace = null))
            .expectStatus().isCreated
    ...
    fun `post-newspapers-titles should use language if present and in Collections`() {
        ...
        createTitle(NewspaperMockData.newspaperTitleInputDtoMockA.copy(language = "nob"))
            .expectStatus().isCreated
    ...
    fun `post-newspapers-titles should create language if present and not in Collections`() {
        every { collectionsRepository.searchLanguage(any()) } returns Mono.just(CollectionsModelMockData.collectionsTermModelWithEmptyRecordListA.copy())
    ...
    fun `post-newspapers-titles should ignore language if not present`() {
        createTitle(NewspaperMockData.newspaperTitleInputDtoMockA.copy(language = null))
            .expectStatus().isCreated
    ...
    fun `post-newspapers-titles should return 400 bad request if start date is after end date`() {
        createTitle(
            NewspaperMockData.newspaperTitleInputDtoMockA.copy(
